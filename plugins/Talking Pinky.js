...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to WAXSON voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
MyPnky.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('WAXSON') && conf.TALKING_WAXSON !== 'true') {        
        ...
        var finm = message.message.replace('WAXSON', '').replace(' ', '')   
...
MyPnky.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_WAXSON == 'true' && ((!message.jid.includes('g.us')) || (message.jid.includes('g.us') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel WAXSON özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'WAXSON yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'WAXSON yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'WAXSON, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'WAXSON, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional WAXSON features. Turn your account into a ai chatbot!'
    already_on = 'WAXSON artificial intelligence is already fully functional.'
    already_off = 'WAXSON artificial intelligence is currently running semi-functional.'
    succ_on = 'WAXSON Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'WAXSON Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ WAXSON സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'WAXSON കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'WAXSON AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'WAXSON പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി WAXSON സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

MyPnky.addCommand({ pattern: 'WAXSON ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.WAXSON on / off' }, (async (message, match) => {
    var WAXSON_status = `${conf.TALKING_WAXSON}`
    if (match[1] == 'on') {
        if (WAXSON_status == 'true') {
